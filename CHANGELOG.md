...
- Changed default `initial_capital` from 10000 to 1000 — I test with smaller amounts closer to what I actually trade with; makes the output feel more grounded. Note: commission impact is proportionally the same, so metrics aren't affected.
- Changed default `commission` from 0.001 (0.1%) to 0.0025 (0.25%) — better reflects the fees I actually pay on my exchange (Kraken spot); the original default was too optimistic for my use case.
